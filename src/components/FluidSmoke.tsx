...
export default function FluidSmoke() {
  ...
  useEffect(() => {
    ...
    // Configuration with reduced smoke size and intensity
    const config = {
      SIM_RESOLUTION: 128,
      DYE_RESOLUTION: 1440,
      CAPTURE_RESOLUTION: 512,
      DENSITY_DISSIPATION: 4.55, // 30% less than original 3.5
      VELOCITY_DISSIPATION: 2.6, // 30% less than original 2
      PRESSURE: 0.1,
      PRESSURE_ITERATIONS: 20,
      CURL: 3,
      SPLAT_RADIUS: 0.12, // 40% less than original 0.2
      SPLAT_FORCE: 4200, // 30% less than original 6000
      SHADING: true,
    ...
    function initializeWebGL() {
      ...
      gl =
        (canvas as any).getContext("webgl2", params) ||
    ...
    function createFBO(w: number, h: number, internalFormat: any, format: number, type: number, param: number) {
    ...
    function createDoubleFBO(w: number, h: number, internalFormat: any, format: number, type: number, param: number) {
    ...
    function clickSplat(pointer: any) {
      ...
      color.g *= 10
      color.b *= 10
      // No spreading on click - zero velocity
      const dx = 0
      const dy = 0
